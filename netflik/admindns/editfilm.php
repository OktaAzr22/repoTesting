        ...
        if ($uploadOk == 0) {
            ...
            if (move_uploaded_file($_FILES["foto"]["tmp_name"], $target_file)) {
                ...
                if (mysqli_query($dbb, $update_query)) {
                    echo "<script>alert('Data berhasil diperbarui'); window.location='film.php';</script>";
                } else {
...
<body>
    ...
    <form method="POST" action="" enctype="multipart/form-data">
        ...
        <input type="time" name="durasi" value="<?php echo htmlspecialchars($data['durasi']); ?>" required><br>

        <label>Sutradara:</label><br>
        <input type="text" name="sutradara" value="<?php echo htmlspecialchars($data['sutradara']); ?>" required><br>

        <label>Tahun Rilis:</label><br>
        <input type="number" name="tahun_rilis" value="<?php echo htmlspecialchars($data['tahun_rilis']); ?>" required><br>

        <label>Rating:</label><br>
        <input type="number" step="0.1" min="0" max="10" name="rating" value="<?php echo htmlspecialchars($data['rating']); ?>" required><br>
