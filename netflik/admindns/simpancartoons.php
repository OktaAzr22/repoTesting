...
if (isset($_POST['simpan'])) {
    ...
    if (move_uploaded_file($_FILES['foto']['tmp_name'], $target_file)) {
        ...
        $query = "INSERT INTO cartoons (judul, durasi, sutradara, tahun_rilis, rating, deskripsi, url_video, foto) VALUES ('$judul', '$durasi', '$sutradara', '$tahun_rilis', '$rating', '$deskripsi', '$url', '$target_file')";

        // Eksekusi query dan periksa hasil
        if (mysqli_query($dbb, $query)) {
            echo "<script>alert('Film berhasil ditambahkan'); window.location='cartoons.php';</script>";
        } else {
