...
$id_seriess = isset($_GET['id_series']) ? $_GET['id_series'] : null;

// Periksa apakah ID film ada
if ($id_seriess) {
    $query = "SELECT * FROM seriess WHERE id_series = ?";
    $stmt = $dbb->prepare($query);
    $stmt->bind_param("i", $id_series);
    $stmt->execute();
    ...
    if ($result->num_rows > 0) {
        ...
        <div class="container">
            ...
            <iframe 
                width="560" 
                height="315" 
                src="<?php echo convertToEmbedUrl($series['url_video']); ?>" 
                frameborder="0" 
                allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" 
                allowfullscreen>
            </iframe>
            <div class="movie-info">
                <img src="<?php echo htmlspecialchars($series['foto']); ?>" alt="Poster" class="poster" onerror="this.src='default-image.jpg';">
                <div class="details">
                    <div class="title"><?php echo htmlspecialchars($series['judul']); ?></div>
                    <div class="rating">Rating: <?php echo $seriess['rating']; ?> 
                        <span class="stars">★★★★★</span>
                    </div>
                    <div class="description"><?php echo htmlspecialchars($series['deskripsi']); ?></div>
                    <div class="additional-info">
                        Sutradara: <?php echo htmlspecialchars($seriess['sutradara']); ?><br>
                        Durasi: <?php echo $series['durasi']; ?><br>
                        Tahun Rilis: <?php echo $series['tahun_rilis']; ?>
                    </div>
                </div>
            </div>
        </div>
        ...
        echo "<p>Seriess tidak ditemukan.</p>";
    }

    $stmt->close();
} else {
    echo "<p>Seriess tidak dipilih.</p>";
}
